...
inline void bswap(T &s)
{
   ...
   switch (sizeof(T))
   {
      case 2: {auto res = __builtin_bswap16(*(uint16_t *)&s); s = *(T *)&res; break; }
      case 4: {auto res = __builtin_bswap32(*(uint32_t *)&s); s = *(T *)&res; break; }
      case 8: {auto res = __builtin_bswap64(*(uint64_t *)&s); s = *(T *)&res; break; }
      default: return;
   }
}
