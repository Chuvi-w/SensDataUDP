...
Socket::Status TcpSocket::connect(const IpAddress& remoteAddress, unsigned short remotePort, Time timeout)
{
   ...
   if(timeout <= Time::Zero)
   {
      ...
      // Connect the socket
      if(::connect(getHandle(), reinterpret_cast<sockaddr*>(&address), sizeof(address)) == -1) return priv::SocketImpl::getErrorStatus();
...
{
   if(!isBlocking()) err() << "Warning: Partial sends might not be handled properly." << std::endl;
...
Socket::Status TcpSocket::send(const void* data, std::size_t size, std::size_t& sent)
{
   ...
   for(sent = 0; sent < size; sent += result)
   {
      ...
      if(result < 0)
      {
         ...
         if((status == NotReady) && sent) return Partial;
...
Socket::Status TcpSocket::send(Packet& packet)
{
   ...
   if(size > 0) std::memcpy(&blockToSend[0] + sizeof(packetSize), data, size);
...
Socket::Status TcpSocket::receive(Packet& packet)
{
   ...
   if(!m_pendingPacket.Data.empty()) packet.onReceive(&m_pendingPacket.Data[0], m_pendingPacket.Data.size());
