...
////////////////////////////////////////////////////////////
bool Ftp::Response::isOk() const { return m_status < 400; }
...
////////////////////////////////////////////////////////////
const std::string& Ftp::DirectoryResponse::getDirectory() const { return m_directory; }
...
////////////////////////////////////////////////////////////
const std::vector<std::string>& Ftp::ListingResponse::getListing() const { return m_listing; }
...
Ftp::Response Ftp::connect(const IpAddress& server, unsigned short port, Time timeout)
{
   ...
   if(m_commandSocket.connect(server, port, timeout) != Socket::Done) return Response(Response::ConnectionFailed);
...
////////////////////////////////////////////////////////////
Ftp::Response Ftp::login() { return login("anonymous", "user@sfml-dev.org"); }

////////////////////////////////////////////////////////////
Ftp::Response Ftp::login(const std::string& name, const std::string& password)
{
   Response response = sendCommand("USER", name);
   if(response.isOk()) response = sendCommand("PASS", password);
...
Ftp::Response Ftp::disconnect()
{
   ...
   if(response.isOk()) m_commandSocket.disconnect();
...
////////////////////////////////////////////////////////////
Ftp::DirectoryResponse Ftp::getWorkingDirectory() { return DirectoryResponse(sendCommand("PWD")); }
...
Ftp::Response Ftp::renameFile(const std::string& file, const std::string& newName)
{
   ...
   if(response.isOk()) response = sendCommand("RNTO", newName);
...
Ftp::Response Ftp::download(const std::string& remoteFile, const std::string& localPath, TransferMode mode)
{
   ...
   if(response.isOk())
   {
      ...
      if(response.isOk())
      {
         ...
         if(!file) return Response(Response::InvalidFile);
         ...
         if(!response.isOk()) std::remove((path + filename).c_str());
      }
   }
...
Ftp::Response Ftp::sendCommand(const std::string& command, const std::string& parameter)
{
   ...
   if(m_commandSocket.send(commandStr.c_str(), commandStr.length()) != Socket::Done) return Response(Response::ConnectionClosed);
...
Ftp::Response Ftp::getResponse()
{
   ...
   for(;;)
   {
      ...
      {
         if(m_commandSocket.receive(buffer, sizeof(buffer), length) != Socket::Done) return Response(Response::ConnectionClosed);
      }
      else
      {
...
Ftp::Response Ftp::DataChannel::open(Ftp::TransferMode mode)
{
   ...
   if(response.isOk())
   {
      ...
      if(begin != std::string::npos)
      {
         ...
         IpAddress      address(static_cast<Uint8>(data[0]), static_cast<Uint8>(data[1]), static_cast<Uint8>(data[2]), static_cast<Uint8>(data[3]));
...
void Ftp::DataChannel::send(std::istream& stream)
{
   ...
   for(;;)
   {
      ...
      if(count > 0)
      {
         ...
         if(m_dataSocket.send(buffer, count) != Socket::Done) break;
      }
      else
      {
