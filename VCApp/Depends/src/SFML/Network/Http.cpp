...
{
   for(std::string::iterator i = str.begin(); i != str.end(); ++i) *i = static_cast<char>(std::tolower(*i));
   return str;
}
} // namespace
...
void Http::Request::setUri(const std::string& uri)
{
   ...
   if(m_uri.empty() || (m_uri[0] != '/')) m_uri.insert(0, "/");
}
...
void Http::Response::parseFields(std::istream& in)
{
   ...
   while(std::getline(in, line) && (line.size() > 2))
   {
      ...
      if(pos != std::string::npos)
      {
         ...
         if(!value.empty() && (*value.rbegin() == '\r')) value.erase(value.size() - 1);
...
void Http::setHost(const std::string& host, unsigned short port)
{
   ...
   if(!m_hostName.empty() && (*m_hostName.rbegin() == '/')) m_hostName.erase(m_hostName.size() - 1);
